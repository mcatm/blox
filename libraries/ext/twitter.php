 ...
 * 
 * Add Some Codes by Satoshi HAMADA (mcatm) in 2010/01/27
 *
 **/
...
class BLX_Twitter {
	...
	private $_methods = array(
					...
					'statuses/retweets'				=> array('http' => 'get',	'auth' => TRUE),
	...
	public function oauth($consumer_key, $consumer_secret, $access_token = NULL, $access_token_secret = NULL, $callback_url = "") {
		$this->deauth();
		$this->oauth = new EpiTwitter($consumer_key, $consumer_secret, $access_token, $access_token_secret, $callback_url);
		$this->oauth->setToken($access_token, $access_token_secret);
		
		if ($access_token === NULL && $access_token_secret === NULL && (!isset($_GET['oauth_token']) || !isset($_GET['oauth_verifier']))) {
			$url = $this->oauth->getAuthorizationUrl();
			header('Location: '.$url);
		} elseif ($access_token === NULL && $access_token_secret === NULL && isset($_GET['oauth_token']) && isset($_GET['oauth_verifier'])) {
			$access_token = $_GET['oauth_token'];
			$access_verifier = $_GET['oauth_verifier'];
			$this->oauth->setToken($access_token, null, $access_verifier);
		...
		} elseif ($access_token !== NULL && $access_token_secret !== NULL) {
			return TRUE;
		}
		return FALSE;
	}
	...
	public function call($method, $params = array()) {
		...
		if ($method != "statuses/retweet" && $method != "statuses/retweets") {
			$url = $this->_url_api . $method . '.' .$this->_api_format;
		} else {
			$url = $this->_url_api . $method . '/' . $params['id'] . '.' .$this->_api_format;
		}
...
class Twitter_Connection {
	...
	public function get($url, $params = array()) {
		...
		if (is_array($params) && !empty($params)) $url = $url . '?' . $this->_params_to_query($params);
		
		$dat = $CI->output->get_cache($url);
		
		if (!$dat) {
			curl_setopt($this->_curl, CURLOPT_URL, $url);
			$dat = curl_exec($this->_curl);
			$expire = ($CI->setting->get('twitter_cache_expire')) ? $CI->setting->get('twitter_cache_expire') : 1;
			$CI->output->set_cache($url, $dat, $expire);
		}
		return $this->deserialize($dat);
	}
	...
	private function _params_to_query($params, $retweet = false) {
...
class EpiTwitter extends EpiOAuth {
	...
	public function __call($name, $params = null) {
		...
		if(!empty($params)) $args = array_shift($params);
	...
	public function __construct($consumer_key, $consumer_secret, $access_token, $access_token_secret, $callback_url = "") {
		...
		parent::__construct($consumerKey, $consumerSecret, self::EPITWITTER_SIGNATURE_METHOD, $callbackUrl);
...
class EpiOAuth {
	...
	public function setToken($token = null, $secret = null, $verifier = null) {
	...
	protected function generateNonce() {
		if(isset($this->nonce)) return $this->nonce;// for unit testing
	...
	protected function prepareParameters($method = null, $url = null, $params = null) {
		...
		$oauth['oauth_verifier'] = $this->tokenVerifier;
		$oauth['oauth_version'] = $this->version;

		//暫定
		if (isset($this->callbackUrl)) $oauth['oauth_callback'] = $this->callbackUrl;
	...
	public function __construct($consumerKey, $consumerSecret, $signatureMethod='HMAC-SHA1', $callbackUrl = "") {
