...
class BLX_feed {
	...
	function read($url) {
		...
		if (!$dat) {
			...
			$p = new MagpieRSS($rss, 'UTF-8', NULL, true);			
			$item = $p->items;
			$CI->output->set_cache($url, compress_array($item), 60);
		} else {
			$item = decompress_array($dat);
		}
		return $item;
	}
...
class MagpieRSS {
	...
	*
	*/
	function MagpieRSS ($source, $output_encoding='UTF-8', $input_encoding=null, $detect_encoding=true) {   
	...
	function feed_start_element($p, $element, &$attrs) {
		...
		$ns = false;
		if (strpos($element, ':')) list($ns, $el) = split(':', $element, 2);
		if ($ns && $ns != 'rdf') $this->current_namespace = $ns;
		...
		if ($el == 'channel') {
			...
			$this->initem = true;
			if (isset($attrs['rdf:about'])) $this->current_item['about'] = $attrs['rdf:about']; 
		}
	...
	function normalize () {
		...
		if ($this->is_atom()) {
			...
			for ($i = 0; $i < count($this->items); $i++) {
				$item = $this->items[$i];
				if (isset($item['summary'])) $item['description'] = $item['summary'];
				if (isset($item['atom_content'])) $item['content']['encoded'] = $item['atom_content'];

				$atom_date = (isset($item['issued']) ) ? $item['issued'] : $item['modified'];
				if ($atom_date) {
					$epoch = @parse_w3cdtf($atom_date);
					if ($epoch and $epoch > 0) $item['date_timestamp'] = $epoch;
				}
			...
			for ($i = 0; $i < count($this->items); $i++) {
				$item = $this->items[$i];
				if (isset($item['description'])) $item['summary'] = $item['description'];
				if (isset($item['content']['encoded'])) $item['atom_content'] = $item['content']['encoded'];
				...
				if ($this->is_rss() == '1.0' && isset($item['dc']['date'])) {
					...
					if ($epoch > 0) $item['date_timestamp'] = $epoch;
				}
	...
	*/
	function php4_create_parser($source, $in_enc, $detect) {
		if (!$detect) return array(xml_parser_create($in_enc), $source);
		...
		if ($this->known_encoding($in_enc)) return array(xml_parser_create($in_enc), $source);
		...
		if (function_exists('iconv'))  {
			...
			if ($encoded_source) return array(xml_parser_create('UTF-8'), $encoded_source);
		}
	...
	function error ($errormsg, $lvl=E_USER_WARNING) {
		...
		if (isset($php_errormsg)) $errormsg .= " ($php_errormsg)";
		if (MAGPIE_DEBUG) {
...
function parse_w3cdtf ( $date_str ) {
	...
	if (preg_match($pat, $date_str, $match)) {
		...
		if ( $match[10] == 'Z' ) {
			...
			#
			if ($tz_mod == '+') $offset_secs = $offset_secs * -1;
