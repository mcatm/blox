...
class Img extends Controller {
	...
	function get_img() {
		...
		if ($param['id'] > 0) {
			...
			$this->db->where('file_id', $param['id']);
			$q = $this->db->get(DB_TBL_FILE);
			$r = $q->result();
			
			if (count($r) > 0) {
				$filename = $param['id'].$r[0]->file_ext;
				$path = $this->file->_make_path($param['id'], FILE_FOLDER);
				$param['path'] = $path['full'].$filename;
				#print_r($path['full'].$filename);
			} else {
				//nopic
			}
			
			if (is_file($param['path'])) {//画像を作成
				$this->load->library('image_lib');
				$this->image_lib->view($param['path'], $param['width'], $param['trim'], $param['height']);
			}
