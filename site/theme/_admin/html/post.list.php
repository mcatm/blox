				...
				<div class="head round">
					...
					<div class="tool">
						<div id="target_tool_filter" class="filter <?if(!isset($this->session->userdata['filter']['admin/'.$post_type]) || count($this->session->userdata['filter']['admin/'.$post_type]) == 0){?> hide<?}?>">
							<div class="sort">
								createdate: <a href="<?=base_url()?>admin/<?=$post_type?>/filter/sort/createdate/desc/">desc</a> | <a href="<?=base_url()?>admin/<?=$post_type?>/filter/sort/createdate/asc/">asc</a> | 
								status: <a href="<?=base_url()?>admin/<?=$post_type?>/filter/where/status/0/"><?=$this->setting->get_status(0)?></a> | <a href="<?=base_url()?>admin/<?=$post_type?>/filter/where/status/1/"><?=$this->setting->get_status(1)?></a> | <a href="<?=base_url()?>admin/<?=$post_type?>/filter/where/status/9/"><?=$this->setting->get_status(9)?></a>
							</div>
							<form action="<?=base_url()?>admin/<?=$post_type?>/filter/q/" method="POST"><input type="text" name="q" value="<?if(isset($this->session->userdata['filter']['admin/'.$post_type]['query'])){?><?=$this->session->userdata['filter']['admin/'.$post_type]['query']?><?}?>" /></form>
							<a href="<?=base_url()?>admin/<?=$post_type?>/filter/clear/">filter clear</a>
						</div>
					</div>
				</div>
